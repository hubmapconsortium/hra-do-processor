...
for schemaFile in `ls schemas/src/{metadata,modules,digital-objects}/*${DO_TYPE}*.yaml`; do
...
for genSchemaFile in schemas/generated/linkml/*${DO_TYPE}*.yaml; do
...
for genSchemaFile in `ls schemas/generated/linkml/*${DO_TYPE}*.yaml | grep -v 'metadata'` schemas/generated/linkml/basic-metadata.yaml; do
  type=$(basename ${genSchemaFile%.yaml})
  echo "## $type" >> $OUT
