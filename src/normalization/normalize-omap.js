...
function normalizeAntibodyData(context, data) {
  return data.map((row) => {
        return new ObjectBuilder() 
          ...
          .append("detects", split(row.HGNC_ID)?.map((text) => normalizeProteinId(text)))
...
function normalizeExperimentData(context, metadata, data) {
  ...
  return new ObjectBuilder()
      ...
      .append("protocol_doi", split(referenceData.protocol_doi)?.map((text) => normalizeDoi(text)))
...
function normalizeExperimentCycleData(context, data) {
  ...
  for (let index = 0; index < cycleNumbers.length; index++) {
    ...
    cycles.push(new ObjectBuilder()
      ...
      .append("uses_antibody", data.filter((row) => row.cycle_number === cycleNumber)
        .map((row) => {
          return new ObjectBuilder()
              ...
              .append("based_on", new ObjectBuilder()
...
function normalizeCoreAntibodyPanelData(context, data) {
  const omapId = data[0].omap_id;
  const antibodyComponents = data.filter((row) => row.core_panel === 'Y')
